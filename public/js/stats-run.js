...
function loadChart() {
    ...
    var numData_min = ['Minimum'];
		...
		// metrics
		var metrics = data.list[0].scoreDetail.metrics;
		for (var i = 1 ; i < data.list.length ; i++) {
			if (data.list[i].scoreDetail.metrics.length > metrics.length) {
				metrics = data.list[i].scoreDetail.metrics;
			}
		}
		...
		for (var i = 0 ; i < data.list.length ; i++) {
			xTicks.push(data.list[i].title);
			numData_score_total.push(data.list[i].score.toFixed(2));
			numData_threshold.push(data.list[i].thresholdInfo.total.toFixed(2));
      numData_min.push(data.list[i].thresholdInfo.hasOwnProperty("min") ? data.list[i].thresholdInfo.min.toFixed(2) : 0);
			// metrics
			metrics = data.list[i].scoreDetail.metrics;
